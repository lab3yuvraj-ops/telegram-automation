...
4. Keep `REPLICATE_API_TOKEN`, `OPENAI_API_KEY`, `ELEVENLABS_API_KEY`, `ELEVENLABS_VOICE_ID`, and `APP_ENCRYPTION_KEY` in `.env`. Enable Replicate billing and `ALLOW_PAID_GENERATION=true` for live films. `TELEGRAM_PRODUCTION_MODE=demo` exercises delivery without external calls.
