...
4. Keep `REPLICATE_API_TOKEN`, `GROQ_API_KEY`, and `APP_ENCRYPTION_KEY` in `.env`. Enable Replicate billing and `ALLOW_PAID_GENERATION=true` for live films. `TELEGRAM_PRODUCTION_MODE=demo` exercises delivery without external calls.
...
`/start`, `/help`, `/id`, `/status`, `/cancel`, `/resume`, `/scene 3`, `/video`, and `/script`. `/scene N` regenerates that scene of the latest stopped film, using paid APIs in live mode.
